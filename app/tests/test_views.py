...
class DashboardTests(LoggedInUserTestCase):
    ...
    def test_message_renders_when_no_skills(self):
        creates_job_role_title_instance()
        response = self.client.get(reverse('dashboard'))
        self.assertIn('Sorry, but at the moment the Junior Developer role has no skills assigned to.',
    ...
    def test_skill_skill_levels_renders(self):
        ...
        self.assertNotIn('Sorry, but at the moment the Junior Developer role has no skills assigned to.',
                         response.content.decode())
