...
class LoginPageTests(TestCase):
    ...
    def test_login_page_valid_credentials(self):
        response = self.client.post(reverse('login'), {'username': 'test@methods.co.uk', 'password': 'test_password'})
        assert response.status_code == 302

    def test_login_page_invalid_credentials(self):
        response = self.client.post(reverse('login'), {'username': 'test@test.com', 'password': 'test_password2'})
        assert response.status_code != 302
